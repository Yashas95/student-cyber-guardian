...
function extractSecurityContext() {
    ...
    const hasMixedContent = location.protocol === "https:" && ([...document.querySelectorAll("img, script, link, iframe")]
        .some(el => {
        const src = el.src
            ?? el.href ?? "";
        return src.startsWith("http://");
...
function showCoachTooltip(input, text) {
    let tooltip = input.parentNode?.querySelector(".scg-coach-tooltip");
    if (!tooltip) {
...
function removeCoachTooltip(input) {
...
function monitorForms() {
    document.querySelectorAll("form").forEach(form => {
        ...
        form.addEventListener("submit", (e) => {
            ...
            chrome.runtime.sendMessage({ type: "CHECK_FORM_SUBMIT", action: form.action, origin: location.origin }, (response) => {
                if (!response?.safe) {
                    e.preventDefault();
...
function showFormWarning(form, reason) {
...
function monitorCryptoAddresses() {
...
function showRiskBanner(result) {
  ...
  `;
    ...
    <div style="display:flex;align-items:flex-start;gap:8px;">
      ...
      <div style="flex:1;">
        ...
        <div id="scg-learn"><ul id="scg-tips">${tips.map((t) => `<li>${t}</li>`).join("")}</ul></div>
      </div>
  ...
  `;
    ...
    const btn = banner.querySelector("#scg-learn-btn");
    const div = banner.querySelector("#scg-learn");
    btn.addEventListener("click", () => {
        const open = div.style.display === "block";
        div.style.display = open ? "none" : "block";
        btn.textContent = open ? "▶ What should I do?" : "▼ What should I do?";
    });
    if (result.riskLevel === "safe" || result.riskLevel === "clean")
        setTimeout(() => banner.remove(), 4000);
}
// ─── Campus Alert Toast (whisper-style, bottom-right) ────────────────────────
function showCampusAlertToast(alert) {
