...
export function detectTechStack(ctx) {
    const found = [];
    for (const rule of TECH_RULES) {
        ...
        if (!matched && rule.scriptPatterns) {
            matched = rule.scriptPatterns.some(pattern => ctx.scriptSrcs.some(src => pattern.test(src)));
        }
        // Check meta tag hints
        if (!matched && rule.metaHints) {
            matched = rule.metaHints.some(hint => ctx.metaTags.some(m => m.name.toLowerCase() === hint.name && hint.pattern.test(m.content)));
        }
        // Check inline script hints
        if (!matched && rule.inlineHints) {
            matched = rule.inlineHints.some(pattern => ctx.scriptInlineHints.some(src => pattern.test(src)));
        }
        if (matched) {
            found.push({
                id: rule.id,
...
export function detectTrackers(ctx) {
    const found = [];
    ...
    for (const rule of TRACKER_RULES) {
        const matched = rule.domains.some(d => allDomains.some(domain => domain === d || domain.endsWith("." + d)));
        if (matched) {
            found.push({
                id: rule.id,
                name: rule.name,
...
export function calculateSecurityChecks(ctx) {
    const checks = [];
    // HTTPS
...
// ─── Grading ──────────────────────────────────────────────────────────────────
export function calculateGrade(checks, trackers) {
    let score = 100;
...
// ─── Friend Summary ───────────────────────────────────────────────────────────
export function buildFriendSummary(hostname, grade, techStack, trackers, checks) {
    ...
    const primaryFrontend = techStack.find(t => ["react", "vue", "angular", "svelte", "nextjs"].includes(t.id));
    let line1;
    let line2;
...
// ─── Lesson of the Page ───────────────────────────────────────────────────────
export function generateLesson(techStack, trackers, checks) {
...
export function runInspectorPipeline(ctx) {
