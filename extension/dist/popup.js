...
import { getCampusId, setCampusId, getCampusAlerts, CAMPUS_LIST, } from "./campusPulse.js";
// ─── Tab switching ─────────────────────────────────────────────────────────────
...
function extractPageContextInTab() {
    ...
    const thirdPartyDomains = [...new Set(scriptSrcs
            .map(src => { try {
            ...
            .filter(h => h && h !== hostname && !h.endsWith("." + hostname)))].slice(0, 40);
    const hasCSP = document.querySelector('meta[http-equiv="Content-Security-Policy"]') !== null;
    const hasMixedContent = location.protocol === "https:" && ([...document.querySelectorAll("img, script, link, iframe")].some(el => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
function buildSecuritySummary(report) {
    ...
    if (https && !https.passed)
        return "⚠️ No HTTPS — take care on this page";
    if (passed === total)
...
async function loadStats() {
    ...
    document.getElementById("stat-safe").textContent = String(stats.safe ?? 0);
    document.getElementById("stat-sus").textContent = String(stats.suspicious ?? 0);
    document.getElementById("stat-danger").textContent = String(stats.dangerous ?? 0);
...
async function loadSettings() {
    ...
    document.getElementById("toggle-scan").checked = s.scanOnNavigate;
    document.getElementById("toggle-badge").checked = s.showBadge;
    document.getElementById("toggle-telemetry").checked = s.telemetryEnabled;
}
...
async function init() {
    ...
    try {
        const cached = await new Promise(resolve => {
            chrome.runtime.sendMessage({ type: "GET_INSPECTOR_REPORT", tabId: tab.id }, (response) => {
...
async function loadCampusPulse() {
    ...
    if (activeAlerts.length === 0) {
        ...
        if (tabDot)
            tabDot.className = "tab-pulse-dot";
    }
    else {
        // Active alerts
        ...
        body.innerHTML = "";
        if (tabDot)
            tabDot.className = "tab-pulse-dot alert";
        alertsContainer.innerHTML = activeAlerts.map(a => {
            const adviceHtml = (a.advice ?? []).map(tip => `<div class="pulse-advice-item">${escHtml(tip)}</div>`).join("");
...
function initCampusSetup() {
    ...
    function renderList(filter) {
        ...
        const filtered = q
            ...
            : CAMPUS_LIST;
        listEl.innerHTML = filtered.map(c => `<div class="campus-list-item" data-campus-id="${c.id}">${escHtml(c.name)}</div>`).join("");
        listEl.querySelectorAll(".campus-list-item").forEach(item => {
            item.addEventListener("click", async () => {
                const id = item.dataset.campusId;
