...
export function analysePasswordStrength(password) {
    ...
    if (/[^a-zA-Z0-9]/.test(password))
        ...
        suggestions.push("Add symbols like !@#$ to make it much stronger.");
    // Penalise common patterns
    if (/^[a-zA-Z]+\d{1,4}$/.test(password)) {
        score -= 15;
        suggestions.push("Don't just add numbers at the end.");
    }
    ...
    const strength = score < 20 ? "very_weak" :
        score < 40 ? "weak" :
            score < 60 ? "fair" :
                score < 80 ? "strong" : "very_strong";
    if (suggestions.length === 0)
        suggestions.push("Good password! Consider using a password manager to remember it.");
...
const seenPasswordHashes = new Map(); // hash → domain
async function sha256(text) {
    const data = new TextEncoder().encode(text);
    const buf = await crypto.subtle.digest("SHA-256", data);
    return [...new Uint8Array(buf)].map(b => b.toString(16).padStart(2, "0")).join("");
}
export async function checkPasswordReuse(password, currentDomain) {
...
export async function fullPasswordAnalysis(password, domain) {
    ...
    if (breach.breached) {
        suggestions.unshift(`🚨 This password has appeared in ${breach.count.toLocaleString()} data breaches. Change it immediately!`);
    }
    if (reuse.reused) {
        suggestions.unshift(`♻️ You used this same password on ${reuse.previousDomain}. Using unique passwords for every site protects you if one gets hacked.`);
    }
    return {
        ...strength,
...
export function checkAutofillSafety(fieldDomain, storedCredentialDomain) {
    if (fieldDomain === storedCredentialDomain) {
        return { safe: true };
    }
    // Allow www prefix mismatch
    const strip = (d) => d.replace(/^www\./, "");
