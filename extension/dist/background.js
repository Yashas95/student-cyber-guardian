...
import { sendThreatSignal, mapThreatCategory, pollCampusAlerts, getCampusAlerts, } from "./campusPulse.js";
 ...
 */
const tabCspHeaderCache = new Map();
// ─── webRequest: capture CSP from HTTP response headers ──────────────────────
chrome.webRequest.onHeadersReceived.addListener((details) => {
    if (details.tabId < 0)
        return; // background requests have tabId -1
    const hasHeader = (details.responseHeaders ?? []).some(h => h.name.toLowerCase() === "content-security-policy");
...
async function updateInspectorBadge(tabId, report) {
    const settings = await getSettings();
    if (!settings.showBadge)
        return;
...
async function maybeShowWhisperToast(tabId, report) {
    // Never show more than once per tab navigation
    if (toastShownTabs.has(tabId))
        return;
...
async function performScan(tabId, url) {
    ...
    if (result.overallSeverity === "high" || result.overallSeverity === "critical") {
        ...
        if (category) {
            sendThreatSignal(url, category).catch(() => { });
        }
    }
...
function extractPageContextForBackground() {
    ...
    const thirdPartyDomains = [...new Set(scriptSrcs.map((src) => { try {
...
async function runInspectorForTab(tabId, url) {
    ...
    try {
        ...
        else if (headerCSP === false && !ctx.hasCSP) {
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
    ...
    if (alarm.name === "pollCampusAlerts") {
        const newAlerts = await pollCampusAlerts().catch(() => []);
        // If any new HIGH or CRITICAL alerts, whisper toast to active tab
        const urgent = newAlerts.filter(a => a.severity === "high" || a.severity === "critical");
        if (urgent.length > 0) {
