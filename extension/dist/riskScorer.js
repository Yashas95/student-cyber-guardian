...
function hasBrandInSubdomain(hostname) {
...
function detectTyposquatting(hostname) {
    ...
    for (const brand of BRAND_KEYWORDS) {
        if (sld === brand)
            continue; // exact match = legit SLD
        const dist = levenshtein(sld, brand);
        if (dist > 0 && dist <= 2 && sld.length > 3)
            return brand;
    }
    return null;
}
// ─── Feature extraction ───────────────────────────────────────────────────────
export function extractFeatures(url, pageText, pageTitle) {
    ...
    const urgencyFound = URGENCY_KEYWORDS.filter(kw => pageText.toLowerCase().includes(kw.toLowerCase()));
    factors.push({
        id: "urgency_language",
...
function buildExplanation(level, factors, hostname) {
...
// ─── Main export ──────────────────────────────────────────────────────────────
export function scanPage(url, pageText = "", pageTitle = "") {
    const parsed = parseUrl(url);
