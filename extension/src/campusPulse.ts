...
export interface ThreatSignal {
    campus_id: string;
    threat_category: "phishing" | "malware" | "scam" | "fake_login";
    domain_hash: string;     // first 8 chars of SHA-256 only
    timestamp: number;       // rounded down to nearest hour
    session_token: string;   // 24h rotating UUID, not linked to identity
}
        ...
        chrome.storage.local.get("pulseSessionMeta", result => {
            ...
            if (meta && now - meta.createdAt < 86_400_000) {
                ...
                chrome.storage.local.set(
                    { pulseSessionMeta: { token: newToken, createdAt: now } },
                    () => resolve(newToken)
                );
            }
        });
...
export async function getCampusId(): Promise<string | null> {
    return new Promise(resolve => {
...
export async function setCampusId(campusId: string): Promise<void> {
    return new Promise(resolve => {
 ...
 */
export function mapThreatCategory(
    primaryThreat: string,
    overallSeverity: string
): ThreatSignal["threat_category"] | null {
    // Only send signals for genuinely dangerous/suspicious detections
    if (overallSeverity !== "high" && overallSeverity !== "critical") return null;

    const mapping: Record<string, ThreatSignal["threat_category"]> = {
        phishing: "phishing",
        malware: "malware",
        scam: "scam",
 ...
 */
export async function sendThreatSignal(
    url: string,
    threatCategory: ThreatSignal["threat_category"]
): Promise<void> {
    try {
        const campusId = await getCampusId();
        if (!campusId) return; // student hasn't set up campus yet — silently skip
 ...
 */
export async function pollCampusAlerts(): Promise<CampusAlert[]> {
    try {
        const campusId = await getCampusId();
        if (!campusId) return [];

        const response = await fetch(
            `${PULSE_SERVER}/pulse/alerts?campus=${encodeURIComponent(campusId)}`
        );
        if (!response.ok) return [];

        const alerts: CampusAlert[] = await response.json();

        // Get previously seen alert IDs
        const prevAlertIds = await new Promise<string[]>(resolve => {
...
export async function getCampusAlerts(): Promise<CampusAlert[]> {
    return new Promise(resolve => {
