...
export interface ThreatSignal {
  ...
  triggered: boolean;
  detail?: string;
  /** Short, jargon-free explanation for the "Teach Moment" UI */
...
export interface ThreatReport {
  ...
  overallSeverity: "low" | "medium" | "high" | "critical" | "clean";
  score: number;           // 0–100
  signals: ThreatSignal[];
  headline: string;        // technical one-liner (kept for backwards compat)
  friendlyHeadline: string; // warm, conversational status message for the UI
  advice: string[];        // 2–3 friendly tips
  timestamp: number;
}
...
function hasHomographIndicators(hostname: string): boolean {
  if (PUNYCODE_RE.test(hostname)) return true;
...
// ─── CSP / security header absence (injected from content script) ─────────────
export interface PageSecurityContext {
  hasCSP: boolean;
  hasMixedContent: boolean;
  hasPasswordField: boolean;
  hasHiddenIframes: number;
...
// ─── Redirect chain abuse ─────────────────────────────────────────────────────
function isRedirectAbuse(redirectCount: number): boolean {
...
export function analyzeThreats(
  url: string,
  ctx: Partial<PageSecurityContext>,
  redirectCount = 0
): ThreatReport {
  ...
  // ── Cryptojacking ────────────────────────────────────────────────────────────
  const cryptoInScripts = scriptSrcs.some(src =>
    CRYPTO_SCRIPT_PATTERNS.some(p => p.test(src))
  );
  ...
  const executableInUrl = MALWARE_EXTENSIONS.has(
  ...
  const passwordOnSuspiciousDomain =
    (ctx.hasPasswordField ?? false) && isSuspiciousDomain(hostname);
  signals.push({
    id: "fake_login_form",
    category: "fake_login",
    severity: "high",
  ...
  const score = Math.min(
    signals.filter(s => s.triggered).reduce((sum, s) => sum + SEVERITY_WEIGHTS[s.severity], 0),
    100
  );
...
function getPrimaryThreat(triggered: ThreatSignal[]): ThreatCategory {
  if (triggered.length === 0) return "clean";
  const priority: ThreatCategory[] = [
    "cryptojacking", "malware", "homograph", "fake_login",
    "data_exfiltration", "phishing", "scam"
  ];
  for (const cat of priority) {
    if (triggered.some(s => s.category === cat)) return cat;
  }
  return triggered[0].category;
}

function getOverallSeverity(score: number): "clean" | "low" | "medium" | "high" | "critical" {
...
const FRIENDLY_HEADLINES: Record<ThreatCategory, string> = {
...
function buildHeadline(cat: ThreatCategory, severity: string, count: number): string {
  const base = HEADLINES[cat];
  if (severity === "clean") return base;
  return `${base} (${count} threat signal${count !== 1 ? "s" : ""} detected)`;
}

function buildFriendlyHeadline(cat: ThreatCategory, severity: string, _count: number): string {
  return FRIENDLY_HEADLINES[cat] ?? FRIENDLY_HEADLINES.clean;
}

const ADVICE: Record<ThreatCategory, string[]> = {
  cryptojacking: [
...
function getAdvice(cat: ThreatCategory, severity: string): string[] {
