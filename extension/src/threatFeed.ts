...
export type FeedResult =
  | { listed: true;  source: string; threat: string; url: string }
...
export async function checkGoogleSafeBrowsing(url: string): Promise<FeedResult> {
  ...
  try {
    ...
    if (!res.ok) return { listed: false, source: "none" };
...
export async function refreshFeedsIfStale(): Promise<void> {
  if (!(await isCacheStale())) return;

  // Fetch URLhaus recent malware URLs (small JSON, ~500 entries)
  try {
    const res = await fetch(URLHAUS_URL, { method: "POST", body: "limit=500" });
    const data = await res.json();
    const urls: string[] = (data.urls ?? []).map((e: { url: string }) => normaliseUrl(e.url));
...
async function getLocalBlocklist(key: string): Promise<Set<string>> {
  return new Promise(resolve => {
    chrome.storage.local.get(key, r => {
      resolve(new Set<string>(r[key] ?? []));
    });
  });
}
...
export async function checkURLhaus(url: string): Promise<FeedResult> {
...
export async function checkAllFeeds(url: string): Promise<FeedResult> {
  ...
  if (gsb.listed)     return gsb;
  if (urlhaus.listed) return urlhaus;
  return { listed: false, source: "none" };
}
